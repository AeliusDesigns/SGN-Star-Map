...
const VS_POINTS = ` attribute vec3 position; uniform mat4 uMVP; uniform float uPointSize; void main(){ gl_Position = uMVP * vec4(position, 1.0); gl_PointSize = uPointSize; }`;
const FS_POINTS = ` precision mediump float; uniform vec3 uColor; void main(){ vec2 uv = gl_PointCoord*2.0 - 1.0; float d = dot(uv,uv); float alpha = smoothstep(1.0, 0.8, 1.0 - d); gl_FragColor = vec4(uColor, alpha); }`;
const VS_LINES = ` attribute vec3 position; uniform mat4 uMVP; void main(){ gl_Position = uMVP * vec4(position, 1.0); }`;
const FS_LINES = ` precision mediump float; uniform vec3 uColor; void main(){ gl_FragColor = vec4(uColor, 1.0); }`;
...
const VS_HALO = ` attribute vec3 position; uniform mat4 uMVP; uniform float uPixelSize; void main(){ gl_Position = uMVP * vec4(position, 1.0); gl_PointSize = uPixelSize; // size in pixels }`;
const FS_HALO = `
...
let defs = ` <defs> <filter id="glow-s" x="-60%" y="-60%" width="220%" height="220%"> <feGaussianBlur stdDeviation="3" result="b"/> <feMerge><feMergeNode in="b"/><feMergeNode in="SourceGraphic"/></feMerge> </filter> <filter id="glow-p" x="-80%" y="-80%" width="260%" height="260%"> <feGaussianBlur stdDeviation="1.5" result="b"/> <feMerge><feMergeNode in="b"/><feMergeNode in="SourceGraphic"/></feMerge> </filter> <radialGradient id="star-grad" cx="40%" cy="35%" r="60%"> <stop offset="0%" stop-color="#fff" stop-opacity="0.9"/> <stop offset="50%" stop-color="${starColor}" stop-opacity="0.9"/> <stop offset="100%" stop-color="${starColor}" stop-opacity="0.5"/> </radialGradient> </defs>`;
...
let hz = ‘’;
if (hzVisible){
hz = `<circle cx="${cx}" cy="${cy}" r="${hzR2.toFixed(1)}" fill="rgba(80,255,120,.06)" stroke="rgba(80,255,120,.18)" stroke-width="0.8" stroke-dasharray="3 4"/> <circle cx="${cx}" cy="${cy}" r="${hzR1.toFixed(1)}" fill="rgba(4,7,14,.0)" stroke="rgba(80,255,120,.12)" stroke-width="0.6" stroke-dasharray="2 5"/>`;
}
...
// Star
const starGlow = `<circle cx="${cx}" cy="${cy}" r="${(starVisR*3.5).toFixed(1)}" fill="${starColor}" opacity=".08"/> <circle cx="${cx}" cy="${cy}" r="${(starVisR*1.8).toFixed(1)}" fill="${starColor}" opacity=".15"/> <circle cx="${cx}" cy="${cy}" r="${starVisR.toFixed(1)}" fill="url(#star-grad)" filter="url(#glow-s)"/>`;
...
const tr = auToR(v);
if (tr > 10 && tr < maxR - 2){
ticks += `<text x="${(cx + tr + 2).toFixed(1)}" y="${(cy + 3).toFixed(1)}" font-family="'Share Tech Mono',monospace" font-size="6" fill="rgba(56,232,255,.3)" letter-spacing="0">${v}AU</text>`;
}
});
...
hzLabel = `<text x="${(cx + midR + 2).toFixed(1)}" y="${(cy - 3).toFixed(1)}" font-family="'Share Tech Mono',monospace" font-size="6" fill="rgba(80,255,120,.45)" letter-spacing="0.5">HZ</text>`;
}
...
pbod.innerHTML = `<div class="bodies-header"> <span class="bodies-count">${planets.length} ORBITAL OBJECT${planets.length!==1?'S':''} DETECTED</span> </div> ${sorted.map(p=>{ const pm = getPlanetMeta(p.type); return`<div class="planet-card">
